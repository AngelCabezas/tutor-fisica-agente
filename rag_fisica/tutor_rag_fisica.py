...
def construir_consulta_teoria(pregunta, resumen_ejercicios):
    ...
    # 1. Movimiento rotacional: prioridad alta
    # Esto evita que "momento" o un p = mv recuperado por error lo mande a cantidad de movimiento.
    if (
        "rueda" in texto
    ...
    # 2. Movimiento de proyectiles: debe evaluarse antes de caída libre
    if (
        "proyectil" in texto
    ...
    # 3. Caída libre, gravedad y vacío
    if (
        "cae" in texto
        or "caen" in texto
        or "caída" in texto
    ...
    # 4. Problemas sin fricción: son Segunda Ley de Newton, no fricción
    if sin_friccion:
        return (
    ...
    # 6. Cantidad de movimiento: antes que energía
    if (
    ...
    # 8. Energía cinética / potencial / conservación
    if (
...
10. Antes de entregar la respuesta, verifica que las ecuaciones no contengan palabras sueltas o texto extraño dentro de las fórmulas, por ejemplo "final" dentro de una potencia o subíndice.
""".strip()
