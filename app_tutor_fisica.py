...
def resolver_pregunta(prompt: str, settings, system_prompt: str) -> None:
    ...
    respuesta_interfaz = buscar_respuesta_interfaz(prompt)

    if respuesta_interfaz.encontrada:
        respuesta_directa = respuesta_interfaz.respuesta
...
def main() -> None:
    ...
    if not st.session_state.messages:
        st.markdown(
            """
            <div style="text-align:center; padding: 40px 20px; opacity: 0.85;">
                <h3>👋 Bienvenido al Tutor de Física</h3>
                <p style="font-size: 1.05rem; max-width: 640px; margin: 0 auto;">
                    Escribe o pega un ejercicio de Física en el cuadro de abajo,
                    o elige uno de los <b>ejemplos rápidos</b> para empezar.
                    El tutor resolverá el problema paso a paso: principio físico,
                    datos, ecuaciones, desarrollo y respuesta final.
                </p>
            </div>
            """,
    ...
    st.markdown(
        """
        <div style="text-align:center; opacity: 0.55; font-size: 0.85rem; padding-top: 8px;">
            Escuela Politécnica Nacional · Facultad de Ingeniería de Sistemas<br>
            Trabajo de Integración Curricular · Angel Vladimir Cabezas Jácome · 2026
        </div>
        """,
        unsafe_allow_html=True,
    )
